...
const GitHubDoc = ({ path, title }) => {
  ...
  React.useEffect(() => {
    const fetchDoc = async () => {
      try {
        const response = await fetch(`https://api.github.com/repos/masumi-network/kodosumi/contents/docs/${path}`);
  ...
  if (loading) {
    return React.createElement('div', 
      ...
      React.createElement('p', null, `🔄 Loading ${title} from GitHub...`)
    );
  }
