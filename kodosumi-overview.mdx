...
try {
const response = await fetch('https://api.github.com/repos/masumi-network/kodosumi/readme');
...
return markdown
.replace(/^# (._$)/gim, '<h1>$1</h1>')
.replace(/^## (._$)/gim, '<h2>$1</h2>')
      .replace(/^### (.*$)/gim, '<h3>$1</h3>')
      .replace(/^#### (.*$)/gim, '<h4>$1</h4>')
      ...
      .replace(/!\[([^\]]*)\]\(([^)]*)\)/gim, (match, alt, src) => {
        const absoluteSrc = src.startsWith('http') 
          ? src 
          : `https://raw.githubusercontent.com/masumi-network/kodosumi/main/${src}`;
        return `<img src="${absoluteSrc}" alt="${alt}" style="max-width: 100%; height: auto; border-radius: 8px; margin: 10px 0;" />`;
})
.replace(/\[([^\]]_)\]\(([^)]_)\)/gim, '<a href="$2" target="_blank" rel="noopener noreferrer" style="color: #16A34A; text-decoration: underline;">$1</a>')
      .replace(/^\- (.*$)/gim, '<li>$1</li>')
...
<div style={{ padding: "20px", textAlign: "center" }}>
  <p>🔄 Loading latest content from GitHub...</p>
</div>
); }
...
<div>
  <Note>
    ...
    <a
      href="https://github.com/masumi-network/kodosumi/blob/main/README.md"
      target="_blank"
