export const GitHubContent = ({ path, branch = 'main', repo = 'masumi-network/kodosumi' }) => {
  ...
  React.useEffect(() => {
    const fetchContent = async () => {
      try {
        const url = `https://api.github.com/repos/${repo}/contents/${path}${branch !== 'main' ? `?ref=${branch}` : ''}`;
